...
- 🎯 Entry points to execute your program using `python -m <monprojet>` or `$ monprojet` with basic CLI argument parsing.
...
---
# monprojet Flask Application

Awesome monprojet created by MarieSOWw
...
```bash
git clone https://github.com/MarieSOWw/monprojet monprojet
cd monprojet
make install
```
...
```bash
monprojet create-db   # run once
monprojet populate-db  # run once (optional)
monprojet add-user -u admin -p 1234  # ads a user
monprojet run
```
