...
original_author="MarieSOWw"
original_name="monprojet"
original_urlname="monprojet"
original_description="Awesome monprojet created by MarieSOWw" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/monprojet "${name}"
...
then
    ...
    echo "then run 'monprojet' or 'python -m monprojet'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'monprojet' or 'python -m monprojet' or 'poetry run monprojet'"
fi
